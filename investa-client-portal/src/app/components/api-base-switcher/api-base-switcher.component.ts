  ...
  template: `
    <div class="fixed bottom-4 end-4 z-50">
      <div class="bg-slate-900 text-white rounded-xl shadow p-3 flex items-center gap-3">
        <div class="text-xs">{{ 'apiSwitcher.apiLabel' | translate }}</div>
        <div class="text-xs font-mono truncate w-48">{{ current() }}</div>
        <button class="text-xs bg-slate-700 px-2 py-1 rounded" (click)="open.set(!open())">{{ 'apiSwitcher.change' | translate }}</button>
      </div>
      <div *ngIf="open()" class="mt-2 bg-white rounded-lg shadow p-3 w-96 text-sm text-black">
        <div *ngFor="let p of presets" class="flex items-center justify-between py-1">
          ...
          <button (click)="apply(p)" class="text-blue-600">{{ 'apiSwitcher.use' | translate }}</button>
        </div>
        <div class="border-t mt-2 pt-2">
          <input [(ngModel)]="custom" [placeholder]="'apiSwitcher.customPlaceholder' | translate" class="w-full rounded border px-2 py-1" />
          <div class="flex justify-end mt-2">
            <button (click)="apply(custom)" class="px-3 py-1 bg-blue-600 text-white rounded">{{ 'apiSwitcher.apply' | translate }}</button>
          </div>
        </div>
      </div>
    </div>
