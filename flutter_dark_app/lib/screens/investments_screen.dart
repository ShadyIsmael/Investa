...
class InvestmentFilter {
  ...
  InvestmentFilter({this.keyword, this.minAmount, this.maxAmount, this.riskLevels, this.status});
}
...
class _InvestmentsScreenState extends State<InvestmentsScreen> {
  ...
  Future<void> _openSearch(BuildContext context, InvestmentFilter? initial) async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: isDark ? AppPalette.midnightDeep : const Color(0xFFF8F9FA),
      appBar: AppBar(
                  ...
                  if (_filterNotifier.value != null)
                    Positioned(
                      ...
                      child: Container(
                        width: 8,
                        height: 8,
                        decoration: BoxDecoration(color: AppPalette.flame, shape: BoxShape.circle),
                      ),
                    )
                ],
              ),
              ...
              children: [
                ...
                Expanded(
                  child: TabBarView(
                    ...
                    children: categories
                        .map((c) => _InvestmentsList(category: c, filterListenable: _filterNotifier))
                        .toList(),
                  ),
                ),
              ],
  ...
  Widget _buildTabBar(BuildContext context, List<Category> categories) {
    ...
    return Container(
      ...
      child: TabBar(
        ...
        unselectedLabelColor: theme.textTheme.bodyMedium?.color?.withOpacityCompat(0.5),
...
class _InvestmentsList extends StatefulWidget {
  ...
  const _InvestmentsList({Key? key, required this.category, this.filterListenable}) : super(key: key);

  @override
  State<_InvestmentsList> createState() => _InvestmentsListState();
}

class _InvestmentsListState extends State<_InvestmentsList> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    if (widget.filterListenable != null) {
      _filterListener = () => setState(() => _applyFilter(widget.filterListenable!.value));
  ...
  void _applyFilter(InvestmentFilter? f) {
    ...
    _filtered = _original.where((it) {
      ...
      if (f.keyword != null && f.keyword!.isNotEmpty) {
        ...
        final title = (item['businessName'] ?? item['title'] ?? '').toString().toLowerCase();
        final desc = (item['description'] ?? item['details'] ?? '').toString().toLowerCase();
      ...
      if (tgt is String) target = double.tryParse(tgt.replaceAll(',', '')) ?? 0.0;
    ...
    if (_filtered.isEmpty && (f.keyword == null && f.minAmount == null && f.maxAmount == null && (f.riskLevels == null || f.riskLevels!.isEmpty) && (f.status == null || f.status!.isEmpty))) {
      _filtered = List.from(_original);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<dynamic>>(
      ...
      builder: (ctx, snap) {
        ...
        if (_filtered.isEmpty && _original.isNotEmpty) _applyFilter(widget.filterListenable?.value);
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                Text('No investments found', style: theme.textTheme.bodyLarge?.copyWith(
...
class _InvestmentCardState extends State<_InvestmentCard> {
  ...
  void _toggleFav() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(_fav ? AppMessages.addedToFavorites : AppMessages.removedFromFavorites), 
      duration: const Duration(seconds: 1)
    ));
  }

  void _share() {
    final title = widget.item['businessName'] ?? widget.item['title'] ?? 'Investment';
  ...
  Future<void> _handleEngage() async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Engage', style: TextStyle(fontWeight: FontWeight.bold))
          ),
        ],
      ),
    );
    ...
    if (confirmed == true && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text(AppMessages.engageRequestSent)));
    }
  }
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (ctx, i) {
                     ...
                     return ListTile(
                       ...
                       trailing: r['score'] != null ? Text('⭐ ${r['score']}') : null,
                     );
                  },
                ),
              )
  ...
  Widget build(BuildContext context) {
    ...
    // Parse raised
    const keys = ['collected', 'collectedAmount', 'raised', 'raisedAmount', 'amountRaised', 'fundedAmount'];
    for (var k in keys) {
      if (item[k] != null) {
        final val = item[k];
        if (val is num) {
        ...
        child: InkWell(
          ...
          onTap: () {
             Navigator.push(context, MaterialPageRoute(
               builder: (_) => InvestmentInfoScreen(item: item, category: widget.category)
             ));
          },
          child: Padding(
            padding: const EdgeInsets.all(20),
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text(
                          'Funded ($percentStr)',
                          style: GoogleFonts.dmSans(fontSize: 12, fontWeight: FontWeight.bold, color: theme.disabledColor),
                        ),
                        RichText(
                          text: TextSpan(
                            style: GoogleFonts.outfit(color: theme.colorScheme.onSurface),
                            children: [
                              TextSpan(text: '$currency${raised.toStringAsFixed(0)} ', style: const TextStyle(fontWeight: FontWeight.bold)),
                              TextSpan(text: '/ $currency${target.toStringAsFixed(0)}', style: TextStyle(color: theme.disabledColor)),
                            ],
                          ),
                        ),
                      ],
                    ...
                    ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                        ...
                        valueColor: AlwaysStoppedAnimation(
                          progress >= 0.75 ? const Color(0xFF10B981) 
                          : progress >= 0.4 ? AppPalette.amber 
                          : AppPalette.flame
                        ),
                      ),
                    ),
  ...
  Widget _buildRiskBadge(String level, ThemeData theme) {
    ...
    switch (level.toUpperCase()) {
      case 'HIGH': color = AppPalette.danger; break;
      case 'LOW': color = const Color(0xFF10B981); break;
      default: color = AppPalette.amber;
    }
    return Container(
...
class _AdvancedSearchSheetState extends State<_AdvancedSearchSheet> {
  ...
  void initState() {
    ...
    _minCtrl = TextEditingController(text: widget.initial?.minAmount?.toString() ?? '');
    _maxCtrl = TextEditingController(text: widget.initial?.maxAmount?.toString() ?? '');
    _risk = widget.initial?.riskLevels != null ? Set.from(widget.initial!.riskLevels!) : {};
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text('Advanced Search', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.w700)),
          ...
          Form(
            ...
            child: Column(
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: TextFormField(controller: _minCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Min amount'))),
                    const SizedBox(width: 12),
                    Expanded(child: TextFormField(controller: _maxCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Max amount'))),
                  ],
                ),
                const SizedBox(height: 12),
                // Risk checkboxes
                Align(alignment: Alignment.centerLeft, child: Text('Risk level', style: theme.textTheme.bodyMedium)),
                Row(
                  children: ['Low', 'Medium', 'High'].map((r) => Padding(
                    padding: const EdgeInsets.only(right: 12.0),
                    child: FilterChip(
                      label: Text(r),
                      selected: _risk.contains(r),
                      onSelected: (s) => setState(() => s ? _risk.add(r) : _risk.remove(r)),
                    ),
                  )).toList(),
                ),
                const SizedBox(height: 12),
