...
public record InvestmentContractSummaryDto(int ContractId, string ContractNumber, Guid InvestorUserId, string FounderDisplayName, string InvestorDisplayName, InvestmentModel InvestmentModel, int CurrentVersionNumber, InvestmentContractStatus Status, DateTime LatestAgreementDate, int VersionCount);
...
public record EmailInvestmentContractResultDto(long EmailOutboxId, Guid OperationId);
