...
class OpportunityStatusUi {
  ...
  /// Normalized canonical keys (UI-side only)
  ...
  static String labelFromRaw(dynamic raw) {
    ...
    switch (normalized) {
      ...
      case 'archived':
        return archived;
      default:
        // Try partial matching for safety
  ...
  /// Status badge colors (consistent across all screens)
  ...
  static Color backgroundFor(dynamic raw) {
    return colorFor(raw).withOpacity(0.14);
  }

  static Color borderFor(dynamic raw) {
    return colorFor(raw).withOpacity(0.28);
  }
  ...
  static String? _normalize(dynamic raw) {
    ...
    // Handle enum-style values (e.g., OpportunityStatus.in_progress)
